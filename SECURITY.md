...
Instead, email **security@openintelligence-labs.org** (replace with the real maintainer alias). Include:
...
We aim to acknowledge within 48 hours and to publish a fix (or detailed mitigation) within 30 days. For high-severity issues we'll request a coordinated disclosure window — typical 90 days from first report.

## Supported versions

| Version | Status |
|---|---|
| 0.x (current) | All supported. We'll backport fixes only to the latest minor. |

We commit to producing a security release for any confirmed high-severity issue. Low-severity issues will be batched into the next regular release.
...
- The Phantom-DB does not contain every unregistered LLM-hallucinated name. That's the point — see [architecture §6.5](docs/ARCHITECTURE.md#65-phantom-db--public-moat-with-responsible-disclosure).
- Network errors during `phantomdep check` cause `UNKNOWN` verdicts (not `BLOCK`). This is documented behaviour to avoid breaking offline workflows.
...
Per architecture §5.5, our MCP server commits to:
